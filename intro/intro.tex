...
Il nous reste à établir une relation entre le repère-caméra et le repère-objet afin d'établir une correspondance entre les propriétés de la scène réelle et sa projection dans l'image. Pour cela, il suffit de déterminer les paramètres du changement de repère.

Soientt ${}^c{\bf P} = ({}^cX, {}^cY, {}^cZ)$ les coordonnées du point ${\bf P}$ exprimées dans le repère $\mathcal R_c$ et ${}^o{\bf P} = ({}^oX, {}^oY, {}^oZ)$ ses coordonnées dans le repère $\mathcal R_o$. Le passage d'un repère à l'autre est effectué en utilisant la relation ${}^c{\bf P} = {}^c{\bf t}_o + {}^c{\bf R}_o {}^o{\bf P}$, avec ${}^c{\bf t}_o$ le vecteur $3\times 1$ de translation entre les deux repères, ${}^c{\bf R}_o$ la matrice $3\times 3$ de rotation correspondant à la rotation des axes du repère. En utilisant les coordonnées homogènes $\widetilde {\bf P}$ de ${\bf P}$, il est possible de factoriser cette transformation en une seule opération matricielle :
\begin{equation}
...
On appelle ${}^c{\bf}_o$ la {\it matrice homogène de transformation} du repère $\mathcal R_o$ au repère $\mathcal R_c$, dont l'inverse s'exprime simplement sous la forme :
\begin{equation}
{}^o{\bf M}_c = 
\begin{bmatrix}
&  {\bf R}^T & & -{\bf R}^T {\bf t} \\
...
\subsubsection{Stratégies d'asservissement}
...
% \subsection{Présentation du formalisme utilisé}
 ...
 %\subsubsection{Stratégies} x3


  %\subsection{interaction blablabla}

    %\subsubsection{Notations} x1

    %\subsubsection{Présentation des stratégie utilisées} x3

    %\section{Comparaisons et justifications} x3
